...
from mmcv.utils import get_logger, Registry, build_from_cfg
...
if __name__ == '__main__':
    """
    possible args:
    --config: something like configs/refine_models/scflow.py
    --load-weights: load or don't load the weights
    """
    ...
    if args.load_weights:
        ...
        logger.info("not loading model weights")
    ...
    if cfg.get('resume_from', None):
        logger.info(f"resuming from `cfg.resume_from`: {cfg.resume_from}")
        runner.resume(cfg.resume_from)
    elif args.resume_from is not None:
        logger.info(f"resuming from `args.resume_from`: {args.resume_from}")
        runner.resume(args.resume_from)
    elif cfg.get('load_from', None):
        logger.info(f"loading checkpoint: {cfg.load_from}")
        runner.load_checkpoint(cfg.load_from)
    else:
        logger.info("not resuming from a checkpoint")
